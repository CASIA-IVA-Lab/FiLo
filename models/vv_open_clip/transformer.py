...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(
        ...
        self.attn_mask = self.attn_mask.to(dtype=q_x.dtype, device=q_x.device) if self.attn_mask is not None else None
    ...
    def __init__(
        ...
        self.resblocks = nn.ModuleList([
            ResidualAttentionBlock(
                width, heads, attn_mask, mlp_ratio, ls_init_value=ls_init_value, act_layer=act_layer, norm_layer=norm_layer,
                idx=idx)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        M = attn_ori.max().item()
        attn_ori = (attn_ori - M).softmax(dim=-1)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_layers: list):
        ...
        if self.attn == None:
            ...
            for i in range(1, self.layers + 1 - 6): # surgery 7, maskclip 2
...
class TextTransformer(nn.Module):
    ...
    def forward(self, text):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        x, attn, patch_tokens = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
